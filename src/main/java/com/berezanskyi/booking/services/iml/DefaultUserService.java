...
public class DefaultUserService implements UserService {
    ...
    public void createUser(User user)  {
        if (findUserByLogin(user.getLogin()).isPresent()){
            throw new BookingException("User with login " + user.getLogin() + " already exists", HttpStatus.BAD_REQUEST);
        }
    ...
    public void editUser(String login, User editUser) {
        User user = userRepository.findByLogin(login)
                .orElseThrow(() -> new BookingException("User with login " + login + " does not exists", HttpStatus.NOT_FOUND));
